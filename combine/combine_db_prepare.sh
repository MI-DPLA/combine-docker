...
echo "### BUILDLOG:  Running /opt/combine/maanage.py createsuperuser" 2>&1 | tee -a $BUILDLOG
python /opt/combine/manage.py createsuperuser 2>&1 | tee -a $BUILDLOG

echo "### BUILDLOG:  Running /opt/combine/maanage.py collectstatic" 2>&1 | tee -a $BUILDLOG
